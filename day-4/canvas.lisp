...
;;        (format t "all-possible: ~A~%" all-possible-positions)
        (when
            (and
            ;; next plot is empty
            (= (aref *maze* (car next-player-position) (cadr next-player-position)) 0)
            ;; no more than 2 possible routes
            (<= (count-if #'(lambda (x) (and (array-in-bounds-p *maze* (car x) (cadr x)) (= (aref *maze* (car x) (cadr x)) 0))) all-possible-positions) 2)
            )
...
(defun on-keydown (key)
  (unless *bgm-started*
    (c:play-wav "assets/bgm.mp3" :loops -1)
    (setf *bgm-started* t))
